...
def setup_record_fixture(app):
    ...
    with app.app_context():
        ...
        PersistentIdentifier.create("recid", "3", status=PIDStatus.DELETED)

        # Record 4 - Registered PID without a record
        PersistentIdentifier.create("recid", "4", status=PIDStatus.REGISTERED)

        # Record 5 - Redirected PID
        pid = PersistentIdentifier.create("recid", "5", status=PIDStatus.REGISTERED)
        pid.redirect(pid1)
        ...
        PersistentIdentifier.create("recid", "7", status=PIDStatus.RESERVED)
        db.session.commit()
...
def test_permission(app):
    ...
    app.config.update(
        ...
        # app.
        RECORDS_UI_DEFAULT_PERMISSION_FACTORY="helpers:" "only_authenticated_users",
    )
    Menu(app)
    InvenioRecordsUI(app)
    app.register_blueprint(create_blueprint_from_app(app))
    ...
    with app.app_context():
        ...
        # Get record 1
        r = Resolver(pid_type="recid", object_type="rec", getter=Record.get_record)
    ...
    with app.test_client() as client:
        ...
        assert res.status_code == 302
        res = client.post(
            login_url, data={"email": "admin@inveniosoftware.org", "password": "123456"}
        )
        assert res.status_code == 302
        res = client.get(record_url)
        res.status_code == 200
...
def test_default_export_format(app, json_v1):
    ...
    default_format = [
        (
            "json",
            {"title": "JSON", "serializer": "fictitious_json_serializer", "order": 1},
        )
    ]
